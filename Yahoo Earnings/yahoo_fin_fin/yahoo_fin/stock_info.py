...
def _parse_earnings_json(url, headers={
    ...
    #resp = requests.get(url, headers=headers, cookies=False)
    #content = resp.content.decode(encoding='utf-8', errors='strict')
...
def get_earnings_history(ticker):
    ...
    first = True
    offset = 0
    while len(result) == 100 or first == True:
        url = 'https://finance.yahoo.com/calendar/earnings/?symbol=' \
              + ticker + '&offset=' + offset.__str__() + '&size=100'
        offset += 100
        ready = False
        while not ready:
            try:
                result = \
                _parse_earnings_json(url)["context"]["dispatcher"]["stores"]["ScreenerResultsStore"]["results"]["rows"]
                ready = True
            except IndexError:
                print("Banned...Retrying in 10 seconds")
                import time
                time.sleep(10)
